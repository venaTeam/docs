...
Keep uses an **asynchronous microservices architecture**. The API handles ingestion (fast), while the Event Handler microservice handles the heavy lifting (parsing, deduplication, workflows).
...
The API layer is responsible **only** for receiving the alert and safely producing it to the messaging layer. It does **not** process, validate, or save the alert to the database synchronously.

### Ingestion Flow
1.  **Endpoint**: Client posts to `POST /alerts/event/{provider_type}`.
2.  **Produce**: The API uses the `EventProducer` to offload the raw event.
    *   If using **Redis**: Pushes to `keep_arq_queue_basic`.
    *   If using **Kafka**: Publishes to the configured topic.
3.  **Response**: Returns `202 Accepted` immediately, confirming receipt.

```mermaid
flowchart LR
    Client(["Client / Webhook"]) -->|POST /event| API[Keep API]
    API -->|Produce| Bus{Messaging Layer}
    Bus -.->|Redis/Kafka| Storage[Queue/Topic]
    API -.->|202 Accepted| Client
    
    style Client fill:#f9f,stroke:#333
    style Bus fill:#ffdec7,stroke:#333
```

## Part 2: Worker Layer (Processing)

The Event Handler service picks up tasks from the messaging layer and executes the business logic. **This is where the actual "work" happens.**

### Processing Flow
1.  **Consume**: The `EventConsumer` (Redis or Kafka) reads the raw message.
...
```mermaid
flowchart TD
    Storage[Queue/Topic] -->|Consume| Worker[Event Handler]
    ...
    style Storage fill:#ffdec7,stroke:#333
```

### Deep Dive: `__handle_formatted_events`

Once the alert is formatted, it enters the orchestration phase provided by `process_event_task.py`:
...
```python keep/common/event_management/process_event_task.py
...
```python
        if event:
            if isinstance(event, AlertDto):
                event = [AlertDto] # Ensure list format
