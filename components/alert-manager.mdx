...
## Alert Audit Trail (`AlertAudit`)

The `AlertAudit` model provides a complete audit history of "who did what and when" for every alert.
...
class AlertAudit(SQLModel, table=True):
    id: UUID
    fingerprint: str           # Links to the alert
    tenant_id: str
    timestamp: datetime        # When the action occurred
    user_id: str               # "system" or actual user email
    action: str                # ActionType enum value
    description: str           # Human-readable text
```

### Audited Actions (`ActionType`)

| **Action** | **Description** |
|------------|-----------------|
| `DEDUPLICATED` | Alert was deduplicated (automated) |
| `TIGGERED` | Alert was triggered |
| `ACKNOWLEDGE` | User acknowledged the alert |
| `AUTOMATIC_RESOLVE` | Alert resolved by system |
| `MANUAL_RESOLVE` | User manually resolved alert |
| `MAPPING_RULE_ENRICH` | Enriched by mapping rule |
| `EXTRACTION_RULE_ENRICH` | Enriched by extraction rule |
| `TICKET_ASSIGNED` / `TICKET_UNASSIGNED` | Ticketing actions |
| `MAINTENANCE` / `MAINTENANCE_EXPIRED` | Maintenance window events |
| `COMMENT` / `UNCOMMENT` | User comments |

### Usage Example

```python
# In process_event_task.py - logging deduplication
audit = AlertAudit(
    tenant_id=tenant_id,
...
Audit logging is controlled by `KEEP_AUDIT_EVENTS_ENABLED`. In high-volume environments (100k+ alerts/hour), disabling this reduces:
- **Database writes**: Each dedup = 1 extra INSERT
- **Storage growth**: The `alertaudit` table grows rapidly
- **Latency**: Reduces transaction time in the hot path

**Trade-off**: Disabling loses the activity timeline, enrichment history, and compliance tracking.

### API Endpoints

- `GET /alerts/{fingerprint}/audit` → Returns `AlertAuditDto` list with grouped consecutive events
...
*   **Audit Model**: `keep/common/models/db/alert.py` (`AlertAudit`)
